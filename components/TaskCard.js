      ...
      <div className={styles.cardHeader}>
        ...
        <div className={styles.cardMeta}>
          ...
          <div className={styles.cardBadges}>
            <span className={styles.badge} style={{ '--badge-bg': cat.bg, '--badge-color': cat.color }}>
              {cat.label}
            </span>
            {task.dueDate && (
      ...
      {total > 0 && (
        <div className={styles.paymentSection}>
          <div className={styles.paymentBar}>
            <div className={`${styles.paymentFill} ${styles[payStatus]}`} style={{ width: `${pct}%` }} />
          </div>
          <div className={styles.paymentRow}>
            ...
            <span className={`${styles.paymentStatus} ${styles[payStatus]}`}>{payLabel}</span>
          </div>
        </div>
      )}

      <div className={styles.cardActions}>
        <button className={styles.actionBtn} onClick={() => onEdit(task)}>✏️ Edit</button>
        <button className={`${styles.actionBtn} ${styles.danger}`} onClick={() => onDelete(task.id)}>🗑 Delete</button>
        <button className={styles.expandBtn} onClick={() => setNotesOpen(o => !o)}>
          Notes <span className={`${styles.expandIcon} ${notesOpen ? styles.open : ''}`}>▾</span>
        </button>
      </div>
        ...
        <div className={styles.notesSectionInner}>
          {editingNotes ? (
            <>
              ...
              <textarea className={styles.notesTextarea} value={notesDraft} onChange={e => setNotesDraft(e.target.value)} rows={3} />
              <div className={styles.notesLabel}>Extra Info / Contacts</div>
              <textarea className={styles.notesTextarea} value={extraDraft} onChange={e => setExtraDraft(e.target.value)} rows={2} />
              <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
                <button className={styles.saveNotesBtn} onClick={handleSaveNotes}>✓ Save</button>
                <button className={styles.saveNotesBtn} style={{ background: '#eee', color: '#666' }} onClick={() => setEditingNotes(false)}>Cancel</button>
              </div>
            </>
          ) : (
            <>
              <div className={styles.notesLabel}>Notes</div>
              <p className={task.notes ? styles.notesText : styles.notesEmpty}>{task.notes || 'No notes yet.'}</p>
              {task.extraInfo && (
                <>
              ...
              <button className={styles.saveNotesBtn} style={{ marginTop: 10 }} onClick={() => setEditingNotes(true)}>✏️ Edit Notes</button>
            </>
          )}
        </div>
      </div>
